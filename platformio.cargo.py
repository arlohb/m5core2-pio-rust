...
class Cargo:
    ...
    def __run_cargo(self, source, target, env):
        ...
        result = env.Execute(f"cargo build {'--release' if self.__cargo_profile == 'release' else ''} --lib --target {self.__rust_target} {self.__cargo_options} -Zbuild-std=std,panic_abort")
